...
from homepage.models import Season1Game
import numpy as np
...
# total games played
def totalGames():
    return (int(Season.objects.latest('gameid').gameid))

# turn order Wins
def turnOrderWins():
    player1wins = str(Season.objects.filter(winner="Player 1").count())
    player2wins = str(Season.objects.filter(winner="Player 2").count())
    player3wins = str(Season.objects.filter(winner="Player 3").count())
    player4wins = str(Season.objects.filter(winner="Player 4").count())
...
def getPlayerList():
    p1 = Season.objects.values(pc=F('p1')).distinct()
    p2 = Season.objects.values(pc=F('p2')).distinct()
    p3 = Season.objects.values(pc=F('p3')).distinct()
    p4 = Season.objects.values(pc=F('p4')).distinct()
    p5 = Season.objects.values(pc=F('p5')).distinct()
    combined = list(chain(p1,p2,p3,p4,p5))
    newcombined = []
    for i in combined:
        new = i['pc']
...
def commandersPlayed(player):
    first = Season.objects.filter(p1=player)
    first = first.values(pc =F('p1c'))
    second = Season.objects.filter(p2=player)
    second = second.values(pc=F('p2c'))
    third = Season.objects.filter(p3=player)
    third = third.values(pc=F('p3c'))
    fourth = Season.objects.filter(p4=player)
    fourth = fourth.values(pc=F('p4c'))
    fifth = Season.objects.filter(p5=player)
    fifth = fifth.values(pc=F('p5c'))
    combined = list(chain(first,second,third,fourth,fifth))
    thisdict = {}
    for i in combined:
        # exists
...
def display(name,thisDict,other):
    ...
    ax1.pie(values1, labels=names1, explode=None, autopct=lambda p: '{:.0f}%({:.0f})'.format(p, (p / 100) * total),
            shadow=False)
    ax1.set_title("Colors")
    fig1.canvas.manager.set_window_title("Magic Boi Stats - " + name)
    fig2, ax2 = plt.subplots()
    ax2.pie(values2, labels=names2, explode=None, autopct=lambda p: '{:.0f}%({:.0f})'.format(p, (p / 100) * total),
            shadow=False)
    ax2.set_title("Commanders")
    fig2.canvas.manager.set_window_title("Magic Boi Stats - " + name)
    plt.show()
