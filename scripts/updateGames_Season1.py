...
def initialLoad():
    with open('./Magic Bois Archive - Season 1.csv', 'r') as file:
        next(file)
        reader = csv.reader(file, skipinitialspace=True)
        cnt = 1
        for row in reader:
            data = Season1Game(gameid = row[0] , date=row[1],p1=row[2],p1c=commanderNames(row[3])
                         ,p2=row[4],p2c=commanderNames(row[5]),p3=row[6],p3c=commanderNames(row[7])
                               ,p4=row[8],p4c=commanderNames(row[9]),p5=row[10],p5c=commanderNames(row[11])
                         ,winner=row[12])
            data.save()
            cnt+=1
